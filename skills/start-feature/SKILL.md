...
Restate the feature in one line and derive `<slug>`. If given a bare `PRE-1234`, fetch the issue title via the Linear MCP rather than guessing. Create `.scratch/<slug>/` and write `progress.md` with all phases pending. Run **`repo-instructions`** (load step) for this repo — if it has a supplements file, hold its rules for Phase 1 and Phase 3.
